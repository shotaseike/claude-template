...
### 1. 新規プロジェクトにコピー（1コマンドで完了）

毎回 GitHub から最新版を取得するため、ローカルキャッシュや事前クローンは不要。

**Dev Container / WSL / Mac / Linux の bash から:**

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/shotaseike/claude-template/main/bootstrap.sh)
```

**Windows PowerShell から（Git for Windows が必要）:**

```powershell
iex (iwr 'https://raw.githubusercontent.com/shotaseike/claude-template/main/bootstrap.ps1').Content
```
