...
git は Windows / Mac / Linux どこでも使えるので OS 非依存。更新時:

```
git -C ~/.claude/claude-template pull
```

### 1. /setup グローバルコマンドを登録（初回のみ）

クローンした `setup.md` をグローバルコマンドとしてリンク:

```bash
mkdir -p ~/.claude/commands
cp ~/.claude/claude-template/.claude/commands/setup.md ~/.claude/commands/setup.md
```

> または Claude Code を起動して「`~/.claude/claude-template/.claude/commands/setup.md` を `~/.claude/commands/setup.md` としてコピーして」と指示するだけでも OK。

### 2. 新規プロジェクトで /setup を実行
