...
```
claude-template/       ← git clone 先 = ~/.claude/claude-template/
├── settings.json               # フック設定
├── commands/
...
cp ~/.claude/claude-template/commands/setup.md ~/.claude/commands/setup.md
```

> または Claude Code を任意のディレクトリで起動して「`~/.claude/claude-template/commands/setup.md` を `~/.claude/commands/setup.md` にコピーして」と言うだけでも OK。
